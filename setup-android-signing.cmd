...
set "BASE64_PS1=%TEMP%\synapse-android-signing-base64-%RANDOM%.ps1"
> "%BASE64_PS1%" echo $ErrorActionPreference = 'Stop'
>> "%BASE64_PS1%" echo [Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false)
>> "%BASE64_PS1%" echo $bytes = [System.IO.File]::ReadAllBytes($env:KEYSTORE_FILE)
>> "%BASE64_PS1%" echo [System.IO.File]::WriteAllText($env:BASE64_FILE, [System.Convert]::ToBase64String($bytes), [System.Text.UTF8Encoding]::new($false))
"%POWERSHELL%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%BASE64_PS1%"
if %errorlevel% neq 0 (
    echo [错误] Base64 编码失败
    if exist "%BASE64_PS1%" del "%BASE64_PS1%" >nul 2>&1
    pause
    exit /b 1
)
if exist "%BASE64_PS1%" del "%BASE64_PS1%" >nul 2>&1
...
set "KEYSTORE_BASE64="
for /f "usebackq delims=" %%b in ("%BASE64_FILE%") do (
...
for %%A in ("%BASE64_FILE%") do echo      长度: %%~zA 字符
echo      值: [已隐藏]
echo.

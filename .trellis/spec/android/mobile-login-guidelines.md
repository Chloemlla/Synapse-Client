...
- Unit tests must cover deep-link ticket, HTTPS callback ticket (with trailing slash), bare query paste (`ticket=...`), provider-bind session, error query, and unrelated HTTPS rejection.

App Links path matching:
- Treat only frontend SPA paths `/auth/linuxdo/callback` and `/auth/provider/bind` as Linux.do related.
- Accept bare query fragments copied from the SPA completion page as ticket/error/session payloads.
...
- Production App Links require `/.well-known/assetlinks.json` to list `com.chloemlla.synapse.mobile` (legacy `com.synapse.mobile` alone is not enough for the production flavor).

...
class SynapseCredentialStore(context: Context) {
    ...
    fun importAccounts(accounts: List<StoredSynapseAccount>, activeAccountId: String?) {
        if (accounts.isEmpty()) {
            clearAll()
            return
        }
        val normalizedActive = activeAccountId
            ?.takeIf { candidate -> accounts.any { it.accountId == candidate } }
            ?: accounts.first().accountId
        saveAccounts(accounts, normalizedActive)
        writeLegacyActive(accounts.firstOrNull { it.accountId == normalizedActive })
    }
